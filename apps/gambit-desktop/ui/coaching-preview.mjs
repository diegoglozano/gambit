...
const point = {
  ...
  played_uci: "g6h7", played_san: "Qh7+", best_san: "Qg8#", best_uci: "g6g8", pv: ["g6g8"], pv_san: ["Qg8#"], after_pv: ["h8h7"], after_pv_san: ["Kxh7"],
...
function summary() {
  ...
  saved.summary = { games_analyzed: ready.length, turning_points: points.length,
    ...
    skipped: points.filter(game => game.record.practice.disposition === "skipped").length,
...
export async function mockCoaching(command, args) {
  ...
  if (command === "coaching_practice") {
    ...
    if (kind === "skip") practice.disposition = "skipped";
  ...
  if (saved) {
    for (const game of saved.games) {
      ...
      game.played_line_positions = game.record?.diagnosis.outcome.kind === "turning_point"
        ? [point.position_fen, "7k/5K1Q/8/8/8/8/8/8 b - - 1 1", "8/5K1k/8/8/8/8/8/8 w - - 0 2"] : [];
