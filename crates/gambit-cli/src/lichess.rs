...
impl fmt::Display for LichessError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotFound(username) => write!(formatter, "user {username:?} was not found"),
...
fn user_games_from(
    ...
    let since;
    if let Some(date) = request.options.since {
        since = date_to_lichess_timestamp(date, false).to_string();
        call = call.query("since", &since);
    }
    let until;
    if let Some(date) = request.options.until {
        until = date_to_lichess_timestamp(date, true).to_string();
    ...
    call.call().map_err(|error| match error {
        ...
        ureq::Error::StatusCode(404) => LichessError::NotFound(request.username.to_owned()),
