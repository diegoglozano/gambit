...
jq -n --arg cpu "$(sysctl -n machdep.cpu.brand_string)" \
  ...
  --arg harness_sha256 "$(shasum -a 256 crates/gambit-engine/examples/review_profile.rs crates/gambit-engine/examples/support/mod.rs crates/gambit-engine/src/lib.rs crates/gambit-chess/src/position.rs | shasum -a 256 | awk '{print $1}')" \
